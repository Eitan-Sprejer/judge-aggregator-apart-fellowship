#!/usr/bin/env python3
"""
Analyze Cross-Model Persona Score Comparison

Loads results from multiple model runs and computes agreement/stability metrics
across different LLMs evaluated on the same dataset with the same personas.

Usage:
    python rebutal_augusto_pt3/analyze_reproducibility.py
    python rebutal_augusto_pt3/analyze_reproducibility.py --results-dir rebutal_augusto_pt3/results
"""
...
def get_model_short_name(model_name):
    """Extract a short human-readable name from a full model identifier."""
    short_names = {
    ...
    return short_names.get(model_name, model_name.split("/")[-1])
...
def load_runs(results_dir):
    ...
    for r in runs:
        r["model_short"] = get_model_short_name(r.get("model", "unknown"))
    print(f"Loaded {len(runs)} runs:")
    for r in runs:
        print(f"  Run {r['run_id']}: {r.get('model', 'unknown')} → {r['model_short']}")
    return runs


def compute_per_sample_stats(runs):
    """Compute per-sample score statistics across models.

    For each sample, computes the mean, std, min, max of its average
    persona score across all model runs.
...
def compute_per_persona_stats(runs):
    """Compute per-persona score statistics across models.
    ...
    for persona in persona_names:
        ...
        # Exact agreement: same score from all models
        exact_agreement = np.sum(per_sample_range == 0) / n_samples * 100

        results.append({
            "persona": persona,
            "mean_score": np.nanmean(matrix),
            "mean_std_across_models": np.nanmean(per_sample_std),
            "mean_range_across_models": np.nanmean(per_sample_range),
...
def compute_pairwise_correlations(runs):
    """Compute Pearson and Spearman correlations between all model pairs.
...
def compute_score_change_stats(score_matrix):
    """Compute statistics about score changes across models.
    ...
    return {
        ...
        "pct_diff_ge_1": float(np.sum(all_diffs >= 1.0) / len(all_diffs) * 100),
        "pct_diff_ge_2": float(np.sum(all_diffs >= 2.0) / len(all_diffs) * 100),
        "pct_exact_same": float(np.sum(all_diffs == 0) / len(all_diffs) * 100),
        "mean_range": float(np.nanmean(ranges)),
        "median_range": float(np.nanmedian(ranges)),
        "pct_zero_range": float(np.sum(ranges == 0) / n_samples * 100),
    }


def plot_score_distributions(runs, output_dir):
    """Plot score distributions for each model as overlaid histograms."""
    ...
    for idx, run in enumerate(runs):
        ...
        ax.hist(scores, bins=30, alpha=0.35, label=run["model_short"],
                density=True, color=color, edgecolor=color, linewidth=0.5)

    ax.set_xlabel("Average Persona Score", fontsize=12)
    ax.set_ylabel("Density", fontsize=12)
    ax.set_title("Score Distributions Across Models", fontsize=14)
...
def plot_per_sample_variance(sample_stats, output_dir):
    """Plot histogram of per-sample standard deviation across models."""
    ...
    axes[0].set_title("Per-Sample Score Variability (Cross-Model)", fontsize=14)
    axes[0].legend()
    ...
    axes[1].set_xlabel("Score Range Across Models (max-min)", fontsize=12)
    axes[1].set_ylabel("Count", fontsize=12)
    axes[1].set_title("Per-Sample Score Range (Cross-Model)", fontsize=14)
    axes[1].legend()
...
def plot_pairwise_scatter(runs, output_dir):
    """Plot scatter plots comparing model pairs."""
    n_runs = len(runs)
    if n_runs < 2:
        return

    # Plot first 10 pairs max
    pairs = [(i, j) for i in range(n_runs) for j in range(i+1, n_runs)][:10]
    n_plots = len(pairs)
    cols = min(3, n_plots)
    ...
    for plot_idx, (i, j) in enumerate(pairs):
        ...
        ax.set_xlabel(runs[i]["model_short"], fontsize=10)
        ax.set_ylabel(runs[j]["model_short"], fontsize=10)
    ...
    fig.suptitle("Pairwise Model Comparisons", fontsize=14)
    fig.tight_layout()
...
def plot_persona_stability(persona_stats, output_dir):
    """Bar chart of per-persona stability metrics across models."""
    fig, axes = plt.subplots(1, 2, figsize=(16, 6))

    # Sort by mean std
    sorted_stats = persona_stats.sort_values("mean_std_across_models", ascending=True)

    # Mean std across models
    axes[0].barh(sorted_stats["persona"], sorted_stats["mean_std_across_models"],
                  color="#4C72B0")
    axes[0].set_xlabel("Mean Std Dev Across Models", fontsize=12)
    axes[0].set_title("Persona Scoring Variability (lower = more consistent)", fontsize=13)
    ...
    axes[1].barh(sorted_stats2["persona"], sorted_stats2["exact_agreement_pct"],
                  color="#55A868")
    axes[1].set_xlabel("% Samples With Exact Same Score Across Models", fontsize=12)
    axes[1].set_title("Cross-Model Exact Agreement", fontsize=13)
...
def plot_correlation_heatmap(corr_matrices, runs, output_dir):
    """Plot correlation heatmaps for Pearson and Spearman with model labels."""
    fig, axes = plt.subplots(1, 2, figsize=(14, 6))
    model_labels = [r["model_short"] for r in runs]
    ...
    fig.suptitle("Cross-Model Correlation", fontsize=14)
    fig.tight_layout()
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze cross-model persona score comparison"
    )
    parser.add_argument(
        "--results-dir", type=str, default="rebutal_augusto_pt3/results",
    ...
    print(f"\n📊 Analyzing {len(runs)} models on {runs[0]['n_samples']} samples each\n")

    # 1. Per-sample stats
    print("1. Per-sample score statistics (cross-model):")
    sample_stats, score_matrix = compute_per_sample_stats(runs)
    print(f"   Mean std across models:   {sample_stats['std'].mean():.4f}")
    print(f"   Median std across models: {sample_stats['std'].median():.4f}")
    print(f"   Mean range across models: {sample_stats['range'].mean():.4f}")
    ...
    print("\n2. Score change magnitude (cross-model):")
    change_stats = compute_score_change_stats(score_matrix)
    print(f"   Mean |Δscore| between model pairs:     {change_stats['mean_abs_diff']:.4f}")
    print(f"   Median |Δscore| between model pairs:   {change_stats['median_abs_diff']:.4f}")
    ...
    print(f"   % samples with same score in ALL models: {change_stats['pct_zero_range']:.1f}%")

    # 3. Per-persona stats
    print("\n3. Per-persona stability (cross-model):")
    ...
    for r in runs:
        valid = [s for s in r["scores"]["average_scores"] if s is not None]
        mean_s = sum(valid) / len(valid) if valid else 0
        print(f"     {r['model_short']:25s}: {mean_s:.3f} (n={len(valid)})")
    ...
    plot_correlation_heatmap(corr_matrices, runs, plots_dir)

    # 6. Save summary
    summary = {
        "n_models": len(runs),
        "n_samples": runs[0]["n_samples"],
        "models": [{"run_id": r["run_id"], "model": r.get("model"), "short": r["model_short"]} for r in runs],
        "temperature": runs[0].get("temperature", 0.7),
        "per_sample": {
    ...
    with open(results_dir / "crossmodel_summary.json", "w") as f:
        json.dump(summary, f, indent=2)

    print(f"\n✅ Analysis complete!")
    print(f"   Summary: {results_dir}/crossmodel_summary.json")
