    ...
    for idx, row in df.iterrows():
        ...
        for p_name, p_data in personas.items():
            ...
            if isinstance(analysis, str) and analysis.startswith("Error"):
                total_errors += 1
                error_details.append(f"Run {run_id}, Sample {idx}, Persona {p_name}: {analysis} | {p_data.get('error', '')}")

print(f"Scanned {total_samples} samples, {total_personas} individual persona scores")
