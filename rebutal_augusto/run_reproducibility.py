#!/usr/bin/env python3
"""
Persona Score Reproducibility Experiment

Runs the persona simulation N times on the same dataset with the same
model and prompts, saving each run separately for later comparison.

Usage:
    # Full dataset, 5 runs (default)
    python rebutal_augusto/run_reproducibility.py

    # Small test (50 samples, 2 runs)
    python rebutal_augusto/run_reproducibility.py --n-samples 50 --n-runs 2

    # Resume from run 3 (if runs 1-2 already done)
    python rebutal_augusto/run_reproducibility.py --resume-from-run 3

    # Use Martian gateway instead of Lambda
    python rebutal_augusto/run_reproducibility.py --api-base https://api.withmartian.com/v1 --api-key-env MARTIAN_API_KEY --model openai/gpt-4o-mini
"""
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=str, default="rebutal_augusto/results",
        help="Output directory (default: rebutal_augusto/results)"
    )
    ...
    print(f"   Next: python rebutal_augusto/analyze_reproducibility.py")
